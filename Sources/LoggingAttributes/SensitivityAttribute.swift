...
extension Logger {
    ...
    /// ])
    /// ```
    public enum Sensitivity: Int64, Sendable, CaseIterable, Equatable, Hashable, CustomStringConvertible,
        MetadataValueAttributes.Attribute
    {
...
// MARK: - MetadataValue sensitivity convenience

extension Logger.MetadataValue {
    /// The sensitivity classification for this metadata value, if set.
    public var sensitivity: Logger.Sensitivity? {
        self.attributes[Logger.Sensitivity.self]
    }
}
...
extension Logger.MetadataValue.StringInterpolation {
    ...
    ///
    /// When a single metadata value contains multiple interpolated segments with
