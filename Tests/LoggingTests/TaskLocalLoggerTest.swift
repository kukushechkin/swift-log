...
struct TaskLocalLoggerTest {
    ...
    // MARK: - Concurrent test execution demo

    @Test func concurrentTest1() async {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })

        await Logger.with(handler: logger.handler) { _ in
            await Logger.with(additionalMetadata: ["test": "1"]) { logger in
                logger.info("concurrent test 1")
            }
        }
    ...
    @Test func concurrentTest2() async {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })

        await Logger.with(handler: logger.handler) { _ in
            await Logger.with(additionalMetadata: ["test": "2"]) { logger in
                logger.info("concurrent test 2")
            }
        }
    ...
    @Test func concurrentTest3() async {
        let logging = TestLogging()
        let logger = Logger(label: "test", factory: { logging.make(label: $0) })

        await Logger.with(handler: logger.handler) { _ in
            await Logger.with(additionalMetadata: ["test": "3"]) { logger in
                logger.info("concurrent test 3")
            }
        }
